...
def generate_cypher_queries():
    ...
    # Return the cypher queries generated from the triplets
    return generate_cypher_from_triplets(triplets)

def generate_cypher_from_triplets(triplets):
    # Example triplet: <triplet><subj>Ukraine</subj><obj>Russia</obj><rel>attacks</rel></triplet>
    cypher_queries = []

    # Regex to find the triplets from the response
    ...
    for subj, obj, rel in matches:
        # Replace spaces and hyphens with underscores to make valid Cypher labels and relationships
        subj = subj.replace(' ', '_').replace('-', '_')
        obj = obj.replace(' ', '_').replace('-', '_')
        rel = rel.replace(' ', '_').upper()  # Replace spaces with underscores for relationships

        # Use the label "Person" for all nodes
        cypher_query = f"""
        MERGE (a:Person {{name: '{subj}'}})
        MERGE (b:Person {{name: '{obj}'}})
        MERGE (a)-[:{rel}]->(b)
        """
        cypher_queries.append(cypher_query)

    # Return the generated Cypher queries
    return "\n".join(cypher_queries)
